package com.sulake.core.window.utils
{
   import com.sulake.core.window.enum.WindowParam;
   import flash.utils.Dictionary;
   
   public class ParamCodeTable extends WindowParam
   {
       
      
      public function ParamCodeTable()
      {
         super();
      }
      
      public static function fillTables(param1:Dictionary, param2:Dictionary = null) : void
      {
         var _loc3_:* = null;
         param1["null"] = const_116;
         param1["bound_to_parent_rect"] = const_81;
         param1["child_window"] = const_959;
         param1["embedded_controller"] = const_304;
         param1["resize_to_accommodate_children"] = const_58;
         param1["input_event_processor"] = const_47;
         param1["internal_event_handling"] = const_726;
         param1["mouse_dragging_target"] = const_226;
         param1["mouse_dragging_trigger"] = const_330;
         param1["mouse_scaling_target"] = const_254;
         param1["mouse_scaling_trigger"] = const_437;
         param1["horizontal_mouse_scaling_trigger"] = const_225;
         param1["vertical_mouse_scaling_trigger"] = const_211;
         param1["observe_parent_input_events"] = const_929;
         param1["optimize_region_to_layout_size"] = const_377;
         param1["parent_window"] = const_968;
         param1["relative_horizontal_scale_center"] = const_162;
         param1["relative_horizontal_scale_fixed"] = const_126;
         param1["relative_horizontal_scale_move"] = const_290;
         param1["relative_horizontal_scale_strech"] = const_269;
         param1["relative_scale_center"] = const_891;
         param1["relative_scale_fixed"] = const_529;
         param1["relative_scale_move"] = const_841;
         param1["relative_scale_strech"] = const_986;
         param1["relative_vertical_scale_center"] = const_159;
         param1["relative_vertical_scale_fixed"] = const_118;
         param1["relative_vertical_scale_move"] = const_287;
         param1["relative_vertical_scale_strech"] = const_259;
         param1["on_resize_align_left"] = const_591;
         param1["on_resize_align_right"] = const_430;
         param1["on_resize_align_center"] = const_401;
         param1["on_resize_align_top"] = const_565;
         param1["on_resize_align_bottom"] = const_429;
         param1["on_resize_align_middle"] = const_464;
         param1["on_accommodate_align_left"] = const_857;
         param1["on_accommodate_align_right"] = const_371;
         param1["on_accommodate_align_center"] = const_699;
         param1["on_accommodate_align_top"] = const_925;
         param1["on_accommodate_align_bottom"] = WINDOW_PARAM_ON_ACCOMMODATE_ALIGN_BOTTOM;
         param1["on_accommodate_align_middle"] = const_566;
         param1["route_input_events_to_parent"] = const_374;
         param1["use_parent_graphic_context"] = const_30;
         param1["draggable_with_mouse"] = const_823;
         param1["scalable_with_mouse"] = const_903;
         param1["reflect_horizontal_resize_to_parent"] = const_365;
         param1["reflect_vertical_resize_to_parent"] = WINDOW_PARAM_REFLECT_VERTICAL_RESIZE_TO_PARENT;
         param1["reflect_resize_to_parent"] = const_237;
         if(param2 != null)
         {
            for(_loc3_ in param1)
            {
               if(param2[param1[_loc3_]] != null)
               {
                  if(param1[_loc3_] != 0)
                  {
                     Logger.log("Conflictiong flags in window params: \"" + _loc3_ + "\"!");
                  }
               }
               param2[param1[_loc3_]] = _loc3_;
            }
         }
      }
   }
}
