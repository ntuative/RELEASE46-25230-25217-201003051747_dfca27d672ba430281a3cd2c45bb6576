package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1393:int = 5;
      ...
      protected var var_2272:String = "";
      
      protected var var_365:int;
      ...
      protected var var_318:URLRequest;
      
      protected var _debug:Boolean = false;
      
      protected var _name:String;
      
      protected var var_338:Class;
      
      protected var var_2379:uint = 0;
      ...
      {
         var_2273 = param1;
         dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR,var_365));
      }
      ...
      protected function prepareLibrary() : void
      {
         ...
         var_338 = this.getDefinition(_name) as Class;
         if(var_338 == null)
         {
            failure("Failed to find resource class \"" + _name + "\" in library " + var_318.url + "!");
            return;
         }
         try
         {
            xmlClass = var_338.manifest as Class;
            if(xmlClass == null)
            {
               return;
            }
      ...
      protected function debug(param1:String) : void
      {
         ...
         if(_debug)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DEBUG,var_365));
         }
      }
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         switch(param1.type)
         {
            case Event.INIT:
               ...
               var_436 |= LibraryLoader.STATE_EVENT_INIT;
               break;
            case Event.COMPLETE:
               debug("Load event COMPLETE for file \"" + var_15.contentLoaderInfo.url + "\"");
               var_436 |= LibraryLoader.STATE_EVENT_COMPLETE;
               break;
            case Event.ENTER_FRAME:
               break;
            case HTTPStatusEvent.HTTP_STATUS:
               var_365 = HTTPStatusEvent(param1).status;
               debug("Load event STATUS " + var_365 + " for file \"" + var_15.contentLoaderInfo.url + "\"");
               break;
            case Event.UNLOAD:
               debug("Load event UNLOAD for file \"" + var_15.contentLoaderInfo.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_UNLOAD,var_365));
               break;
            case ProgressEvent.PROGRESS:
               debug("Load event PROGRESS for file \"" + var_15.contentLoaderInfo.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_PROGRESS,var_365));
               break;
            case IOErrorEvent.IO_ERROR:
               failure("IO Error, send or load operation failed for file \"" + var_15.contentLoaderInfo.url + "\"");
               if(!handleHttpStatus(var_365))
               {
         ...
         if(var_436 == LibraryLoader.STATE_ANALYZE)
         {
            if(analyzeLibrary())
            {
               var_436 |= LibraryLoader.STATE_EVENT_FRAME;
            }
         }
         if(var_436 == LibraryLoader.STATE_READY)
         {
            var_1375 = true;
            prepareLibrary();
            removeEventListeners();
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE,var_365));
         }
      }
      ...
      protected function handleHttpStatus(param1:int) : Boolean
      {
         if(param1 >= 400)
         {
            if(var_932 > 0)
            {
               var_15.close();
               var_15.unload();
               addRequestCounterToUrlRequest(var_318,const_1393 - var_932);
               var_15.load(var_318,_context);
               --var_932;
               return true;
            }
