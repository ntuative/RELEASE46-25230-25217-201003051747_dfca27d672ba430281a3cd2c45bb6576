   ...
   public class EventDispatcher implements IEventDispatcher, IDisposable
   {
      ...
      public function dispatchEvent(param1:Event) : Boolean
      {
         if(!_disposed)
         {
            ...
            if(_result == const_748)
            {
               if(var_636 != null)
               {
                  Core.crash("Error catched in EventDispatcher: undefined",var_636.errorID,var_636);
               }
               else
               {
                  Core.crash("Error catched in EventDispatcher. No error data available!",0,var_636);
               }
            }
      ...
      public function removeEventListener(param1:String, param2:Function, param3:Boolean = false) : void
      {
         ...
         if(!_disposed)
         {
            _loc4_ = var_245[param1];
            if(_loc4_)
            {
               _loc5_ = 0;
               for each(_loc6_ in _loc4_)
               {
                  if(_loc6_.callback == param2 && _loc6_.var_2252 == param3)
                  {
                     _loc4_.splice(_loc5_,1);
                     _loc6_.callback = null;
                     if(_loc4_.length == 0)
                     {
                        delete var_245[param1];
                     }
                     return;
                  }
                  _loc5_++;
      ...
      public function addEventListener(param1:String, param2:Function, param3:Boolean = false, param4:int = 0, param5:Boolean = false) : void
      {
         ...
         else
         {
            ...
            {
               if(_loc8_.callback == param2 && _loc8_.var_2252 == param3)
               {
                  return;
               }
      ...
      private function eventProcessor(param1:Event) : void
      {
         ...
         var array:Array = var_245[event.type];
         if(array)
         {
            callbacks = [];
         ...
         _result = !!event.isDefaultPrevented() ? uint(const_1048) : uint(const_488);
      }
      ...
      public function dispose() : void
      {
         ...
         if(!_disposed)
         {
            for(_loc1_ in var_245)
            {
               _loc2_ = var_245[_loc1_] as Array;
               for each(_loc3_ in _loc2_)
               {
                  _loc3_.callback = null;
               }
               delete var_245[_loc1_];
            }
            var_245 = null;
