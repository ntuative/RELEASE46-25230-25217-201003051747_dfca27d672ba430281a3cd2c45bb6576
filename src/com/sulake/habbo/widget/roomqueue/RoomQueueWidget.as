package com.sulake.habbo.widget.roomqueue
{
   ...
   public class RoomQueueWidget extends RoomWidgetBase
   {
      ...
      private function changeQueue(param1:WindowMouseEvent) : void
      {
         ...
         if(var_1378 == RoomWidgetRoomQueueUpdateEvent.const_317)
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE);
         }
         else
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_577);
         }
      ...
      private function onQueueStatus(param1:RoomWidgetRoomQueueUpdateEvent) : void
      {
         ...
         if(param1.isActive)
         {
            var_1378 = param1.type;
            var_935 = false;
            var_936 = param1.position;
         }
         else
         {
            var_935 = true;
         }
         var_1606 = param1.hasHabboClub;
         var_1377 = param1.isClubQueue;
         localizations.registerParameter("room.queue.position","position",var_936.toString());
         localizations.registerParameter("room.queue.position.hc","position",var_936.toString());
         localizations.registerParameter("room.queue.spectator.position","position",var_936.toString());
         localizations.registerParameter("room.queue.spectator.position.hc","position",var_936.toString());
         showInterface();
      }
      ...
      override public function registerUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.addEventListener(RoomWidgetRoomQueueUpdateEvent.const_317,onQueueStatus);
      ...
      private function exitQueue(param1:WindowMouseEvent) : void
      {
         ...
         var _loc2_:RoomWidgetRoomQueueMessage = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_597);
         messageListener.processWidgetMessage(_loc2_);
         removeWindow();
      }
      ...
      override public function unregisterUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.removeEventListener(RoomWidgetRoomQueueUpdateEvent.const_317,onQueueStatus);
      ...
      {
         messageListener.processWidgetMessage(new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_588));
      }
      ...
      private function showInterface() : void
      {
         ...
         if(_loc1_ != null && _loc2_ != null && _loc3_ != null)
         {
            switch(var_1378)
            {
               case RoomWidgetRoomQueueUpdateEvent.const_317:
                  _loc1_.caption = !!var_1377 ? "${room.queue.position.hc}" : "${room.queue.position}";
                  _loc2_.caption = "${room.queue.spectatormode}";
                  _loc3_.visible = var_935;
                  break;
               case RoomWidgetRoomQueueUpdateEvent.const_472:
                  _loc1_.caption = !!var_1377 ? "${room.queue.spectator.position.hc}" : "${room.queue.spectator.position}";
                  _loc2_.caption = "${room.queue.back}";
                  _loc3_.visible = false;
            }
            _loc2_.visible = var_935;
         }
         var _loc4_:IWindowContainer = _window.findChildByName("club_container") as IWindowContainer;
         if(_loc4_ != null)
         {
            _loc4_.visible = !var_1606;
         }
