package com.sulake.habbo.avatar.cache
{
   ...
   public class AvatarImageDirectionCache
   {
      ...
      public function comparePartList(param1:Array) : Boolean
      {
         ...
         if(var_670.length == param1.length)
         {
            _loc3_ = 0;
            while(_loc3_ < var_670.length)
            {
               _loc4_ = var_670[_loc3_];
               _loc5_ = var_670[_loc3_];
               if(_loc4_ != _loc5_)
               {
                  _loc2_ = false;
                  break;
