package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(var_380 >= var_293.length)
         {
            return "";
         }
         var petColor:IPetColor = var_293[var_380] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = var_558[var_1432] as IBreed;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         var_380 = param1.index;
         if(var_380 < 0 || var_380 > var_293.length)
         {
            var_380 = 0;
         }
         updateImage();
      ...
      private function onApproveNameResult(param1:CatalogWidgetApproveNameResultEvent) : void
      {
         ...
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_585.getWithIndex(0),page,p);
      }
      ...
      override public function init() : void
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_146,onTextWindowEvent);
         var_585 = new Map();
         ...
         var_788 = getPetTypeIndexFromProduct(_loc5_.localizationId);
         var_293 = [];
         var_558 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_788);
               if(_loc11_)
               {
                  var_558 = _loc11_.breeds;
                  var_293 = _loc11_.colors;
                  if(var_293 == null)
                  {
                     var_293 = [];
                  }
         ...
         var _loc6_:int = 0;
         while(_loc6_ < var_558.length)
         {
            _loc12_ = var_558[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_788,_loc12_.id),getRaceLocalizationKey(var_788,_loc12_.id)));
            _loc6_++;
         }
         var_585.add(var_788,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_910,onApproveNameResult);
         _loc5_ = var_585.getWithIndex(0);
         if(_loc5_ != null)
         {
            events.dispatchEvent(new SelectProductEvent(_loc5_));
         }
         var _loc8_:* = [];
         for each(_loc9_ in var_293)
         {
      ...
      {
         var _loc7_:* = null;
         var _loc1_:Offer = var_585.getWithIndex(0);
         if(_loc1_ == null)
         {
            return;
         }
         ...
         if(var_380 >= 0 && var_380 < var_293.length)
         {
            _loc7_ = var_293[var_380] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = var_558[var_1432] as IBreed;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImageNew(var_788,_loc3_.id,_loc2_,AvatarScaleType.const_51);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_278,AvatarAction.const_697);
